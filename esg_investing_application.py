...
ttk.Label(root, text="\n EcoRise - an ESG Asset Allocation Application", font="italic").grid(row=0,columnspan=2)
...
def get_portfolio():
    ...
    def getFamaFrench3_returns(stocks):
        ...
        # Read Fama-French data and filter the same time period as stock data:
        ff_data = pd.read_csv('fama_french_data.csv', index_col='date', parse_dates=True)
        ff_data = ff_data.loc[(ff_data.index > '2017') & (ff_data.index < '2022-04')]
        ff_data = ff_data.resample('M').last()
        
        # Align stock prices to the same period and resample to monthly frequency:
        stocks = stocks.loc[(stocks.index > '2017') & (stocks.index < '2022-04')].resample('M').last()
        
        # Ensure both dataframes are aligned by date:
        common_dates = stocks.index.intersection(ff_data.index)
        stocks = stocks.loc[common_dates]
        ff_data = ff_data.loc[common_dates]
        
        # Calculate excess returns (subtract risk-free rates):
        excess = stocks.pct_change().fillna(method='ffill').fillna(0)
        excess = excess.subtract(ff_data['RF'], axis=0)
        
        # Set regression variables (three factors + constant):
        factors = ff_data[['Mkt-RF', 'SMB', 'HML']]
        factors = sm.add_constant(factors)
        
        # Perform regression for each stock separately:
        ff_betas = {}
        for stock in excess.columns:
            y = excess[stock]  # Excess return for the specific stock
            model = sm.OLS(y, factors, missing='drop')  # Handle missing data
            results = model.fit()
            ff_betas[stock] = results.params  # Store regression coefficients
        
        # Convert `ff_betas` dictionary to DataFrame:
        ff_betas_df = pd.DataFrame(ff_betas).T  # Transpose for stocks as rows
        ff_betas_df.columns = ['const', 'b1', 'b2', 'b3']  # Rename columns for clarity
        
        # Calculate expected returns using Fama-French model equation:
        r_f = ff_data['RF'].mean()                     # Average risk-free rate
        ...
        e_r = (
            r_f +
            ff_betas_df['b1'] * mkt_prem +
            ff_betas_df['b2'] * SMB +
            ff_betas_df['b3'] * HML
        ) * 12  # Annualized returns
        
        # Return a Series with stock names as index and expected returns as values:
        return pd.Series(e_r, index=ff_betas_df.index)
    ...
    def BlackLit_opt(prices, risk_a):
        ...
        Q = getFamaFrench3_returns(prices)  # Ensure getFamaFrench3_returns returns a pandas Series
        # print(type(Q))
        # print("Helllo",Q)
        if isinstance(Q, np.ndarray):
        # Convert Q to a pandas Series using the columns of `prices` as index
            Q = pd.Series(Q, index=prices.columns)
        Q = Q[Q > -1]  # Filter invalid predictions

        # Ensure `prices` contains only stocks for which we have predictions in `Q`:
        prices = prices.loc[:, Q.index]  # Filter price data to match prediction data
        ...
        mcap_wgts = (mcap_data / mcap_data.sum())['CUR_MKT_CAP'].values  # Convert to numpy array
        ...
        P = np.zeros((len(Q), len(prices.columns)))  # Ensure the matrix matches the dimensions
        np.fill_diagonal(P, 1)  # Fill the diagonal with 1s for each stock
        ...
        # Black-Litterman Excess Return: (Meucci formula)
        r2 = np.linalg.inv(tau * P @ S @ P.T + omega)
        post_pi = pi + (tau * S @ P.T) @ r2 @ (Q.values - P @ pi)

        # Black-Litterman Covariance Matrix: (Meucci formula)
        c2 = np.linalg.inv(tau * P @ S @ P.T + omega)
        post_S = (1.0 + tau) * S - tau**2.0 * S @ P.T @ c2 @ P @ S

        symS = (post_S + post_S.T) / 2  # Make it symmetric
        semidefS = cov_nearest(symS)  # Ensure strict positive semi-definiteness
    ...
    plt.savefig("portfolio_chart.jpg")
